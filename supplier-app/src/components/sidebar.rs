...
pub fn Sidebar(user: User) -> impl IntoView {
    let logout = create_action(move |_| async move {
        let auth_client = use_context::<AuthClient>().expect("missing auth client");
        let result = auth_client.create_browser_logout_flow().await;

        match result {
            Ok(logout_flow) => {
                auth_client
                    .redirect_to_logout_url(logout_flow.logout_url)
                    .await;
            }
